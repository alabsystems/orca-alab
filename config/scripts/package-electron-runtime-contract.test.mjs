...
// pnpm 10 ignores package.json's `pnpm` field — 76f4914ac ("stop pnpm silently
// ignoring four settings") moved these to pnpm-workspace.yaml, which is now the
// only source-build allowlist pnpm honors. Verified against pnpm 10.24.0 with no
// `pnpm` field left in package.json.
const honoredBuiltDependencyAllowlist = () => {
  const allowlist = parse(readProjectFile('pnpm-workspace.yaml')).onlyBuiltDependencies
  // Asserting ABSENCE, not just ignoring it: a re-added `pnpm` field would be a
  // shadow allowlist that reads as authoritative and is silently ignored.
  expect(packageJson.pnpm, 'pnpm 10 ignores this field; use pnpm-workspace.yaml').toBeUndefined()
  // Anti-vacuity: if the allowlist moves homes again, every `not.toContain` below
  // would otherwise pass against an empty set while pnpm builds whatever it likes.
...
describe('Electron runtime package contract', () => {
  it('publishes package metadata for the renamed development repository', () => {
    expect(packageJson.homepage).toBe('https://github.com/alabsystems/orca-alab')
    expect(packageJson.repository).toEqual({
      type: 'git',
      url: 'git+https://github.com/alabsystems/orca-alab.git'
    })
    expect(packageJson.bugs).toEqual({
      url: 'https://github.com/alabsystems/orca-alab/issues'
    })
  })
  ...
  it('keeps the native Windows registry addon optional and platform-gated', () => {
    const rebuildScript = readProjectFile('config/scripts/rebuild-native-deps.mjs')
    const ensureScript = readProjectFile('config/scripts/ensure-native-runtime.mjs')
  ...
  it.skipIf(!HAS_CI_WORKFLOWS)('packages and release-gates the SSH relay watcher child', () => {
    const relayBuild = readProjectFile('config/scripts/build-relay.mjs')
  ...
  it('packages and verifies the Windows SSH node-pty console-list fallback', () => {
    ...
    // Fork names the digest builder `hashBuilder`: it also folds in the
    // node-pty-patched payload copied for every platform.
    expect(relayBuild).toContain(
  ...
  it.skipIf(!HAS_CI_WORKFLOWS)('publishes both Linux release matrix entries', () => {
    const releaseWorkflow = readProjectFile('.github/workflows/release-cut.yml')
    const parsedWorkflow = parse(releaseWorkflow)
  ...
  it.skipIf(!HAS_CI_WORKFLOWS)(
    'installs the Electron package binary in PR checks without changing native module ABI',
    () => {
  ...
  it.skipIf(!HAS_CI_WORKFLOWS)(
    'smokes the packaged CLI from outside the checkout in PR checks',
    () => {
      const prWorkflow = readProjectFile('.github/workflows/pr.yml')
      const parsedWorkflow = parse(prWorkflow)
